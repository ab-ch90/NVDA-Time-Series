...
def forecast_next_days(last_values, model, lags=10, days=5): # given last few days of prices, predict the next days future prices one at a time
    ...
    for _ in range(days): # run prediciton loop once per future day
        ...
        x_df = pd.DataFrame([x], columns=[f"lag_{i}" for i in range(1, lags+1)]) # convert input into a DataFrame, single-row table 
        yhat = model.predict(x_df)[0]
        preds.append(yhat)
        history.append(yhat)
    return preds 
...
def main(days=5):
    ...
    preds = forecast_next_days(last_values, model, lags=lags, days=days)
